...
$('#startButton').on("click", function() {
    if ($("#newUser").val() != "") {
        console.log("start");
        newUser = $("#newUser").val();
        console.log(playerOneExist);
        console.log(currentPlayers + "cur players");
        createNewUser();
    }
});
/*create new user*/
function createNewUser() {
    if (currentPlayers < 2) {
        ...
        userRef = database.ref('/users/' + playerNum);
        //creates player
        ...
        //this will revome the player if they get disconected
        setRPS();
        userRef.onDisconnect().remove();
        // If a user disconnects, set the current turn to 'null' so the game does not continue
        turnRef.onDisconnect().remove();
        //displayes welcome blank! you are player (1 or 2)
        $('.userInfo').html(
            '<h2> Hi ' + newUser + '! You are Player ' + playerNum + '</h2>'
        );
    } else {
...
playersRef.on('value', function(snapshot) {
    ...
    console.log('curPlayer ' + currentPlayers);
